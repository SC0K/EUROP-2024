% Version 4 - adding CBF to the MPC - works fine
...
%% MPC and CBF
            for c = 1:ly
                ...
                x_mpc = sdpvar(nx,N+1, nd);
                    
                for k = 1:N
%                    hk = (X_mpc(1,1) - X(1,combinations(d,c),t))^2 + (X_mpc(2,1) - X(2,combinations(d,c),t))^2;     % 2 norm
%                     X_mpc = A{s}*X_mpc+B{s}*u{k};
%                     constraints = [constraints, -a_lim <= u{k}, u{k} <= a_lim];
...
%                     hk1 = (X(1,combinations(d,c),1) - X(1,combinations(d,c),t))^2 + (X(2,combinations(d,c),1) - X(2,combinations(d,c),t))^2 ;     % 2 norm wrt to initial condition
%                     constraints = [constraints, 0 <= hk1 + 2*(X(1:2,combinations(d,c),1)-X(1:2,combinations(d,c),1))'*(X_mpc(1:2,1) - X(1:2,combinations(d,c),1)) -gamma*hk + r1^2*(gamma-1)];
                
                %%%%%% Method 2 %%%%%%%%%
                constraints = [constraints, -a_lim <= u{k}, u{k} <= a_lim];
                constraints = [constraints, x_mpc(:,k+1,d) == A{s}*x_mpc(:,k,d) + B{s}*u{k}];
                constraints = [constraints, x_mpc(:,1,d) == X(:,d,t)];
                hk = (x_mpc(1,k,d) - X(1,combinations(d,c),t))^2 + (x_mpc(2,k,d) - X(2,combinations(d,c),t))^2;
                hk1 = (X(1,combinations(d,c),1) - X(1,combinations(d,c),t))^2 + (X(2,combinations(d,c),1) - X(2,combinations(d,c),t))^2 ;     % 2 norm wrt to initial condition
                constraints = [constraints, 0 <= hk1 + 2*(X(1:2,combinations(d,c),1)-X(1:2,combinations(d,c),1))'*(x_mpc(1:2,k+1,d) - X(1:2,combinations(d,c),1)) -gamma*hk + r1^2*(gamma-1)];
...
%% Simulation
options = sdpsettings('verbose', 1,'cache', -1,'solver','quadprog');
