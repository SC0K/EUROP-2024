...
%% MPC and CBF
            for c = 1:ly
                for k = 1:N
                    hk0 = abs(X_mpc(1,1) - X(1,combinations(d,c),t))/r1 + abs(X_mpc(2,1) - X(2,combinations(d,c),t))/r2 - 1;
                    constraints = [constraints, hk0 >= 0];  
                    hk = abs(X_mpc(1,1) - X(1,combinations(d,c),t))/r1 + abs(X_mpc(2,1) - X(2,combinations(d,c),t))/r2 - 1;     % Barrier function at k
...
%                     constraints = [constraints, -a_lim*ones(nu,1) <= u{k} <= a_lim*ones(nu,1)];
                    ...
                    hk1 = abs(X_mpc(1,1) - X(1,combinations(d,c),t))/r1 + abs(X_mpc(2,1) - X(2,combinations(d,c),t))/r2 - 1;     % Barrier function at k+1
