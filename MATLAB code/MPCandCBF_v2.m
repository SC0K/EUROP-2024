%Version 2 - optimiser method - MPC working but not with the CBF constraints
yalmip('clear')
clear all
...
%              % Target destination
%              if even(d)
%                 objective{d} = objective{d} + (x_mega{s}{k}(:,d)-X(:,d-1,1))'*Q*(x_mega{s}{k}(:,d)-X(:,d-1,1))+ u_mega{s}{k}(:,d)'*R*u_mega{s}{k}(:,d);
%              else
%                  objective{d} = objective{d} + (x_mega{s}{k}(:,d)-X(:,d+1,1))'*Q*(x_mega{s}{k}(:,d)-X(:,d+1,1))+ u_mega{s}{k}(:,d)'*R*u_mega{s}{k}(:,d);
%              end
%         end
%         if even(d)
%             objective{d} = objective{d} + eta*(x_mega{s}{N+1}(:,d)-X(:,d-1,1))'*QN{s}*(x_mega{s}{N}(:,d)-X(:,d-1,1));
%         else
%             objective{d} = objective{d} + eta*(x_mega{s}{N+1}(:,d)-X(:,d+1,1))'*QN{s}*(x_mega{s}{N}(:,d)-X(:,d+1,1));    % Termination state
%         end
%     end
% end
...
for t = 1:T+1
    ...
    fprintf(' drone: %d, at time step: %d, input u: %f,%f\n', d, t,u(1,1),u(2,1));
