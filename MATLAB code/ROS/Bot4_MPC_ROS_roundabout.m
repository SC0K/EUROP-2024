...
% ROS Publishers
cmd_accel_x_pub2 = rospublisher('/robot4/accel_x', 'std_msgs/Float32');
cmd_accel_y_pub2 = rospublisher('/robot4/accel_y', 'std_msgs/Float32');

% ROS Subscribers
odom_sub1 = rossubscriber('/robot4/odom', 'nav_msgs/Odometry', @odom_callback1);
...
while true
    ...
    if all(state1(:)) ~= 0 && all(state2(:)) ~= 0
        ...
        if all(~isnan(U2(:))) && all(~isnan(U2_1(:)))
            ...
            dist2 = norm(state1(1:2) - targets(1:2, 2));
    
            if dist2 < threshold
                disp('Robot4 have reached its target.');
                break;
            end
            ...
            send(cmd_accel_y_pub2, msg_y2);
            pause(h);
            States_history4 = [States_history4; state1(1), state1(2)];
        end
    end
end
