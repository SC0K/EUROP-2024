...
%DI_CONTROLLER1 - Safety constraints to all inputs
    ...
    for k = 1: N
        ...
        hkk = (state_main_update(1)^2 + state_main_update(2)^2)-  (r_roundabout+ 0.5 - D/2)^2;  % Roundabout constraint
        ...
        if abs(state_main(2))< D/2 && abs(state_main(1)) > r_roundabout 
            constraints = [constraints, -D/2<=state_main_update(2)<=D/2];
        else
            constraints = [constraints, 0 <= hkk1 - hkk+ gamma*hkk, 0 >= hkk1 - hkk+ gamma*hkk];
        end
    end
