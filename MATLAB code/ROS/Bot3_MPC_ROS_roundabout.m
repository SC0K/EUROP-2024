...
% ROS Publishers
cmd_accel_x_pub2 = rospublisher('/robot3/accel_x', 'std_msgs/Float32');
cmd_accel_y_pub2 = rospublisher('/robot3/accel_y', 'std_msgs/Float32');

% ROS Subscribers
odom_sub1 = rossubscriber('/robot3/odom', 'nav_msgs/Odometry', @odom_callback1);
...
while true
    ...
    if all(state1(:)) ~= 0 && all(state2(:)) ~= 0
        tic
        [U2,U2_1,feas] = DI_controller1_vert(state1, state2, N, A0, B0, Q, R, QN, r1, r2, gamma, eta, a_lim, Bd{1}, disturbance(:, :, 1), targets(:, 1), 2.75, 0.2);
        ...
        if all(~isnan(U2(:))) && all(~isnan(U2_1(:)))
            ...
            dist2 = norm(state1(1:2) - targets(1:2, 1));
    
            if dist2 < threshold
                disp('Robot3 have reached its target.');
                break;
            end
            ...
            send(cmd_accel_y_pub2, msg_y2);
            pause(h);
            States_history3 = [States_history3; state1(1), state1(2)];
        end
    end
end
